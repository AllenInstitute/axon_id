...
def parent_rad(seg, feat_df, upstream_df, soma_rad=8000):
    ...
    try:
        upseg = upstream_df[upstream_df["downstream_seg"] == seg]["upstream_seg"].iloc[
            0
        ]
...
def parent_class(seg, class_df, downstream_segment_graph):
    try:
        ...
        upstream_seg = downstream_segment_graph[
            downstream_segment_graph["downstream_seg"] == seg
        ]["upstream_seg"].iloc[0]
...
def downstream_segment_graph(
    sk, columns=["upstream_seg", "downstream_seg"], sortcol="upstream_seg"
):
    """
...
def segment_graph(skel, df):
    ...
    creates a graph of each segment-neighbor pair, where each segment is represented by
    the segment centerpoint df must have 'ctr_pt' and 'classification
    """
    ...
    # what about when a -> b & c & d. need to indicate that b and c and d are all
    ...
    for skelidx in list(seg_connectivity_df["a"].unique()):
        ...
        connected_list = list(
            seg_connectivity_df.loc[seg_connectivity_df["a"] == skelidx, "b"]
        )

        # find the combinations of all of those downstream segments, as they are all
        ...
        for i in range(len(combodf)):
            combodf.loc[i, "class a"] = seg_connectivity_df.loc[
                seg_connectivity_df["b"] == combodf.loc[i, "a"], ["class b"]
            ].iloc[0, 0]
            combodf.loc[i, "class b"] = seg_connectivity_df.loc[
                seg_connectivity_df["b"] == combodf.loc[i, "b"], ["class b"]
            ].iloc[0, 0]

            # add the combo df to the end of the connectivity df
            running_combo_df.append(combodf)

    # add the running combo df (which contains all combos) to the seg connectivity df
    seg_connectivity_df = pd.concat([seg_connectivity_df] + running_combo_df)

    # we are making this an undirected graph, so add the b columns to a and their
    # corresponding a columns to b
    reversed_seg_connectivity_df = seg_connectivity_df.copy()
    reversed_seg_connectivity_df.loc[
        :, ["a", "class a", "b", "class b"]
    ] = seg_connectivity_df.loc[:, ["b", "class b", "a", "class a"]].values

    # add the two graphs together
    final_seg_connectivity_df = pd.concat(
        [seg_connectivity_df, reversed_seg_connectivity_df]
    )
...
def neighboring_segments(skel, df):
    ...
    takes in a mesh and its corresponding segment features and classification df and
    adds a column with the classification of neighboring segments to each segment.
    ...
    for i in range(len(df)):
        ...
        # use segment connectivity graph to find the number of connected segments of
        # each class for each segment in this skeleton
        df.loc[i, "dendrite neighbor count"] = len(
            segment_connectivity_graph.loc[
                (segment_connectivity_graph.loc[:, "a"] == ctrpt)
                & (segment_connectivity_graph.loc[:, "class b"] == 0.0)
            ]
        )
        df.loc[i, "axon neighbor count"] = len(
            segment_connectivity_graph.loc[
                (segment_connectivity_graph.loc[:, "a"] == ctrpt)
                & (segment_connectivity_graph.loc[:, "class b"] == 1.0)
            ]
        )
