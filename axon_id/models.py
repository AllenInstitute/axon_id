...
def extract_features(mw, soma_seg_len=10000):
    ...
    for i in range(len(feat_df)):
        ...
        feat_df.at[i, "pre"] = np.intersect1d(
            seg, mw.skeleton.mesh_to_skel_map[pre_msh_inds]
        )
        ...
        feat_df.at[i, "post"] = np.intersect1d(
            seg, mw.skeleton.mesh_to_skel_map[post_msh_inds]
        )
        ...
        feat_df.loc[i, "density"] = (
            feat_df.loc[i, "total_syn"] / feat_df.loc[i, "length"]
        )
        feat_df.loc[i, "soma_dist"] = mw.skeleton.path_length(
            mw.skeleton.path_between(int(mw.skeleton.root), seg[-1])
        )

        feat_df.loc[i, "radius"] = np.mean(
            seg_df.loc[seg_df.index.isin(msh_seg)]["r_eff"]
        )

        feat_df.loc[i, "endpoint"] = bool([x for x in seg if x in endpts])

    # now add neighbor feats
    upstream_df = graph.downstream_segment_graph(
        mw.skeleton, columns=["upstream_seg", "downstream_seg"], sortcol="upstream_seg"
    )
    feat_df["self/parent_rad"] = [
        graph.parent_rad(seg, feat_df, upstream_df) for seg in range(len(feat_df))
    ]

    return feat_df


def classify_axon_dendrite(
    mw,
    model_path,
    model_columns=[
        "length",
...
def classify_axon_dendrite_2(
    ...
    m1_df = classify_axon_dendrite(
        mw, model1_path, model_columns=model1_columns, soma_seg_len=10000
    )
...
def extract_features_2(mw, m1_class_df):
    ...
    r2_df["upstream_class"] = [
        graph.parent_class(seg, m1_class_df, downstream_seg_graph)
        for seg in r2_df.index
    ]
...
def class_1_2_axons(
    ...
    class_df = classify_axon_dendrite_2(
        mw,
        m1,
        m2,
        model2_columns=model2_columns,
    ...
    skel_ax_verts = mw.SkeletonIndex(
        skel_mask_df[skel_mask_df["tf dendrite"] == False]["skeleton index"]
    )
    mesh_ax_verts = skel_ax_verts.to_mesh_index
    # save the annotation
    mw.anno.add_annotations(axon_class_anno, mesh_ax_verts, mask=True, overwrite=True)
...
def find_primary_axon(
    ...
    axon_inds, Q = meshwork.algorithms.split_axon_by_annotation(
        mw, pre_anno=pre_anno, post_anno=post_anno
    )

    if Q >= certainty_threshold:
        mw.anno.add_annotations(primary_axon_anno, axon_inds, mask=True, overwrite=True)
    else:
        mw.anno.add_annotations(
            primary_axon_anno, mw.MeshIndex([]), mask=True, overwrite=True
        )
...
def combine_primary_and_axon_class(
    mw,
    axon_class_anno="is_axon_class",
    primary_axon_anno="is_primary_axon",
    ...
    all_axon = mw.MeshIndex(
        list(
            set(mw.anno[axon_class_anno]["mesh_index"])
            | set(mw.anno[primary_axon_anno]["mesh_index"])
        )
    )

    mw.anno.add_annotations(ax_agglom_anno, all_axon, mask=True, overwrite=True)

    if mask_out_ax:
        mw.apply_mask(~mw.anno[ax_agglom_anno].mesh_mask)


def add_class_12_primary_anno(
    mw,
    m1,
    m2,
    model2_columns=[
    ...
    class_1_2_axons(
        ...
        axon_class_anno=axon_class_anno,
    )
    ...
    # combine these two
    combine_primary_and_axon_class(
        mw,
        axon_class_anno=axon_class_anno,
        primary_axon_anno=primary_axon_anno,
